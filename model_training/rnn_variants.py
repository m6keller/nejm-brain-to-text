...
class LSTMDecoder(nn.Module):
    '''
    LSTM-based decoder variant with day-specific input layers.
    Similar setup as GRUDecoder but using LSTM cells.
    '''
    def __init__(self, neuraldim, nunits, ndays, nclasses,
        ...
        self.out = nn.Linear(nunits, nclasses)
        nn.init.xavier_uniform_(self.out.weight)

        # Learnable initial states for hidden and cell
        self.h0 = nn.Parameter(torch.zeros(nlayers, 1, nunits))
        self.c0 = nn.Parameter(torch.zeros(nlayers, 1, nunits))

    def forward(self, x, dayidx, states=None, returnstate=False):
        # x shape: (batch, time, features)
        dayweights = torch.stack([self.dayweights[i] for i in dayidx], dim=0)
        daybiases = torch.cat([self.daybiases[i] for i in dayidx], dim=0).unsqueeze(1)
        ...
        if self.patchsize > 0:
            # Same patching logic as original GRUDecoder
        ...
        if states is None:
            ...
            h0 = self.h0.expand(self.nlayers, batch_size, self.nunits).contiguous()
            c0 = self.c0.expand(self.nlayers, batch_size, self.nunits).contiguous()
            states = (h0, c0)
